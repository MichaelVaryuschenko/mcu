...
def main():
	ser = serial.Serial(port='COM29', baudrate=115200, timeout=0.0)
	if (ser.is_open):
	...
	try:
		while True:
			...
			voltage_V, temperature_C = read_value(ser)
			measure_ts.append(ts)
			measure_voltage_V.append(voltage_V)
			measure_temperature_C.append(temperature_C)
			print(f'{voltage_V:.3f} V - {temperature_C:.1f}C - {ts:.2f}s')
			time.sleep(0.1)
	finally:
		ser.write("tm_stop\n".encode('ascii'))
		ser.close()
